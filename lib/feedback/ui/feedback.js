...
(function (global) {
  ...
  function issuePreviewHtml(issue, env) {
    if (!issue) return '';
    var html = '<div class="tg-feedback-preview-title">Issue 草稿（确认无误再提交）</div>';
    html += '<div class="tg-feedback-preview-item"><b>标题：</b>' + esc(issue.title || '（无）') + '</div>';
    if (issue.description) html += '<div class="tg-feedback-preview-item"><b>描述：</b>' + esc(issue.description) + '</div>';
    if (issue.steps && issue.steps.length) {
      html += '<div class="tg-feedback-preview-item"><b>复现步骤：</b><ol>' + issue.steps.map(function (s) { return '<li>' + esc(s) + '</li>'; }).join('') + '</ol></div>';
    }
    if (issue.expected) html += '<div class="tg-feedback-preview-item"><b>期望：</b>' + esc(issue.expected) + '</div>';
    if (issue.actual) html += '<div class="tg-feedback-preview-item"><b>实际：</b>' + esc(issue.actual) + '</div>';
    var envText = renderEnvText(env);
    if (envText) html += '<details class="tg-feedback-preview-env"><summary>环境信息</summary><pre>' + esc(envText) + '</pre></details>';
    return html;
  }

  function feedbackHtml() {
    return ''
      + '<button type="button" class="tg-feedback-open" id="fb-open-btn">反馈</button>'
      + '<div class="tg-feedback-modal" id="fb-modal" hidden role="dialog" aria-modal="true" aria-labelledby="fb-modal-title">'
      +   '<div class="tg-feedback-backdrop" data-fb-close></div>'
      +   '<div class="tg-feedback-dialog">'
      +     '<div class="tg-feedback-head">'
      +       '<div class="tg-feedback-titles"><span class="tg-feedback-title" id="fb-modal-title">反馈小助手</span><span class="tg-feedback-sub">想说啥用大白话讲，它帮你整理成规范反馈</span></div>'
      +       '<button type="button" class="tg-feedback-close" data-fb-close aria-label="关闭反馈窗口">×</button>'
      +     '</div>'
      +     '<div class="tg-feedback-messages" id="fb-messages"></div>'
      +     '<div class="tg-feedback-preview" id="fb-issue-preview" hidden></div>'
      +     '<div class="tg-feedback-input-row">'
      +       '<textarea id="fb-input" placeholder="比如：更新完设置就打不开了…" rows="2"></textarea>'
      +       '<button type="button" class="tg-feedback-send" id="fb-send-btn">发送</button>'
      +     '</div>'
      +     '<div class="tg-feedback-actions" id="fb-actions" hidden>'
      +       '<a class="tg-feedback-btn tg-feedback-btn-primary" id="fb-submit-link" target="_blank" rel="noopener" hidden>生成提交页</a>'
      +       '<button type="button" class="tg-feedback-btn" id="fb-copy-btn">复制文案</button>'
      +       '<span class="tg-feedback-actions-hint" id="fb-actions-hint">检查整理好的内容后再提交</span>'
      +     '</div>'
      +   '</div>'
      + '</div>';
  }
  ...
  function bindFeedback(opts) {
    ...
    function bubble(role, text) {
      ...
      div.className = 'tg-feedback-bubble tg-feedback-bubble-' + role;
    ...
    function resetChat() {
      ...
      lastIssue = null;
      lastEnv = null;
      messagesEl.innerHTML = '<div class="tg-feedback-welcome">告诉我遇到了什么问题，或者想要什么新功能。\n不清楚的地方它会问你。</div>';
