...
test("ModelConfig：hana 档带 providerId/modelId 且不碰密钥", async () => {
  const store = makeStore({ modelSource: "hana", hanaModel: { providerId: "p1", modelId: "m1" } });
  const ctx = makeCtx();
  const mc = new ModelConfig({ ctx, store });
  await mc.sample([{ role: "user", content: "hi" }]);
  const call = ctx._calls[0];
  assert.equal(call.payload.providerId, "p1");
  assert.equal(call.payload.modelId, "m1");
  assert.ok(!("apiKey" in call.payload));
});
