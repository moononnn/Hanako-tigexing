...
/**
 * 解析会话标题（带短重试）：新会话第一轮回复完成时，宿主的异步起名可能还没落地
 * （标题要等回复完成后才生成），查不到就等一小会儿再查一次，仍查不到返回 null。
 */
export async function resolveSessionTitleWithRetry({ bus, agentsHome, agentId, sessionPath, sessionId, retryDelayMs = 1500 } = {}) {
  let title = await resolveSessionTitle({ bus, agentsHome, agentId, sessionPath, sessionId });
  if (!title) {
    await new Promise((resolve) => setTimeout(resolve, retryDelayMs));
    title = await resolveSessionTitle({ bus, agentsHome, agentId, sessionPath, sessionId });
  }
  return title;
}
