 ...
 * @param {string} [opts.duration] 系统通知时长："default"=系统默认 / "long"=停留久一些
 * @param {object} [opts.log] ctx.log，可选
 * @returns {boolean} 是否成功发起
 */
export function sendToast({ pluginDir, title, message, style, icon, sound, duration, log }) {
  ...
  const env = {
    ...
    TXS_DURATION: duration === "long" ? "long" : ""
  };
